...
#+name: note_name
#+begin_src scheme :var notes=note_names :results output
    <<log2_func>>
    <<octave_reduction>>
    (define (note-name n d)
      (let* ((R (octave-reduction (rationalize (inexact->exact (/ n d)) 1/100)))
             (c (* 1200 (log2 (octave-reduction R))))
             (n (inexact->exact (truncate (/ (+ c 50) 100)))))
        (car (cdr (assoc n notes)))))
  ;;      n))
  ;;(format #t "~a" (note-name 3 4))
#+end_src
...
#+begin_src scheme :results output :noweb yes :var ellis=ellis_signs notes=note_names
  ...
  (format #t "~a~a" (note-name 3 4) (retrieve-ellis-ext 5/4))
#+end_src

#+RESULTS: retrieve_ellis_extension
: <stdin>:24:30: warning: possibly unbound variable `true'
: <stdin>:25:64: warning: possibly unbound variable `false'
: asoaa

#+begin_src scheme :noweb yes :results output :var tbl=ellis_signs  notes=note_names
  <<note_name>>
  (format #t "~a~a" (note-name 5 4) (note-name 5 4))
#+end_src
