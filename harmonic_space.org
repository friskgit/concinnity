...
~radArrays = MatrixArray.with([[6, 3, 3], [3, 3, 6], [6, 3, 3], [3, 6, 3], [3, 3, 3], [3, 3, 3], [6, 3, 3], [6, 6, 3], [6, 6, 3], [3, 3, 3], [9, 9, 6], [9, 6, 6], [6, 3, 3], [3, 0, 6], [9, 6, 3], [9, 3, 3], [3, 6, 3], [6, 6, 3], [6, 3, 6], [9, 6, 6]]);
#+end_SRC
...
#+begin_src sclang :tangle harmonic_space_gui.scd
  (
  var sax, synth, master, mute, currentInputVal, wind, text, textC, textB, textD, mapA, mapB, step, plus, minus, number, annotX, annotY, annotZ, time, start, myClock, input, meters;
  wind = Window.new.front;
  mapA = ControlSpec(0, 4, \linear, 0.01); // min, max, mapping, step
...
#+begin_src sclang :results none :noweb yes :tangle link harmonic_space_main.scd
  <<synth_main>>
  <<shift_main>>
  <<change_chord>>
#+end_src
    ...
    if(gate == 0,
          {
            ...
            ~distExpandA.value(outs, chord, ~spatArrays.rowAt(ndx), ~elArrays.rowAt(ndx), ~radArrays.rowAt(ndx), amps, rrand(3,6));
          },
          {
            "Playing Pbind B".postln;	  	  
            ~distExpandB.value(outs, chord, ~spatArrays.rowAt(ndx), ~elArrays.rowAt(ndx), ~radArrays.rowAt(ndx), amps, rrand(3,6));
          }
    );
  };
  ...
  ~distExpandA = { |out, chord, spat, el, rad, amp, reps|
    ...
    fork {
          pbA = Pbind(\instrument, "chordPlayer",
            ...
            \rad, Pseq(rad, reps),
            \amp, Pseq(mAmp, reps),
            \length, Pseq([len, len], reps) * Prand([(1..4)], reps),
          ...
          pbB = Pbind(\instrument, "chordPlayer",
            ...
            \rad, Pseq(rad, reps),
            \amp, Pseq(mAmp, reps),
            \length, Pseq([len, len], inf) * Prand([(1..4)], reps),
            \combv, Prand([0.1, 0.11, 0.12, 0.13, 0.15, 0.17, 0.19, 0.2], reps),   
...
#+name: dist_expand_b
#+begin_src sclang
  ~distExpandB = { |out, chord, spat, el, rad, amp, rep|
  ...
  ~distExpandB.value([ 0, 0, 0 ], [ 654.80625, 523.845, 436.5375 ], [ 0.78539816, 1.5707963, 0.0 ], [ 0.62831853, 0, 0 ], [ 6, 3, 3 ], [ 0.1, 0.1, 0.1 ]);
#+end_src
  ...
  ~shifters.do({ arg obj, int;
    ...
    obj.source = {
          ...
          PitchShiftPA.ar(
            ...
            \maxFormantRatio.kr(10), \grainsPeriod.kr(2)) * \shiftVol.kr(1.5);
    };
  });
#+end_src
...
#+begin_src sclang :results none
  ...
  ~encoders.do({ arg obj, int;
    obj.fadeTime = ~fadeTime;
    obj.source = {
          HOAEncoder.ar(~order, \in.ar(0), \az.kr(1.7), \el.kr(0), 0.5, \spherical.ir(1), \rad.kr(1), 1.07);
    };
  });
