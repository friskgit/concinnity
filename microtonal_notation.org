     ...
     (define (eh-printer R)
       (begin
         ...
         (format #t "~a = {\n  ~a\n  ~a\n" "music" "\\accidentalStyle \"dodecaphonic\"" "\\fixed c' {")
          (map (lambda (note)
  ;;               (let* ((N (note-name note #t)))
                   (format #t "    ~a~a~a\n"
                           (car (note-name note #t))
                           (retrieve-ellis-ext note)
                           (if (unspecified? (octave-notation (cdr (note-name note #t))))
                               ""
                               (octave-notation (cdr (note-name note #t))))))
                 R)
         (format #t "  ~a\n~a\n\n" "}" "}")
         (format #t "~a\n  ~a\n    ~a  ~a\n" "\\score {" "\\new Staff {" "\\new Voice {" "\\music")
         (format #t "    ~a\n  ~a\n  ~a\n  ~a\n"  "}" "}" "\\layout{}\n  \\midi{}" "}")
    ...
    (eh-printer '(1/1 2/1 3/1 4/1 5/1 6/1))
#+end_src
